...
const CurrentShowCard = ({
  ...
  return (
    <div
      className="card w-4/5 flex items-stretch bg-[var(--colour-background-secondary)] rounded-[42px] overflow-hidden font-[var(--font-sans)]"
    ...
    >
      ...
      <div className="flex items-center gap-10 w-full py-12 px-12">
        ...
        <div className="flex flex-col justify-between h-full flex-1">
          <h3 className="text-[48px] font-bold leading-tight">{title}</h3>
          <p className="text-[20px] leading-relaxed pb-[50px]">{description}</p>
          <div className="bg-[var(--colour-background-white)] rounded-2xl px-5 py-4 pr-30 mt-1 shadow-sm">
            ...
            {[dates, time, doors, location].map((line) => (
              <p key={line} className="text-[20px] leading-snug">
                {line}
              </p>
            ))}
          </div>
